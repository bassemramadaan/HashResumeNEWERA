        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((t, idx) => (
            <motion.div
              ...
              key={idx}
              className="bg-white rounded-2xl p-6 shadow-xs border border-slate-200/60 hover:border-brand-600/30 hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col relative overflow-hidden group"
            >
              {/* Ambient micro border glow line */}
              <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-brand-600 to-[#000a1b] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              ...
              <div className="flex items-center gap-3 border-t border-slate-100/80 pt-4">
                <div className="w-9 h-9 rounded-full bg-brand-600/10 text-brand-600 flex items-center justify-center font-black shrink-0 text-xs">
                  {t.initials}
                </div>
                <div>
