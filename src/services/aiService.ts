  ...
  generateContent: async (prompt: string, systemInstruction?: string): Promise<AIResponse> => {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (errorMessage.includes("API key not valid") || errorMessage.includes("SAFETY")) {
          break;
        }
    ...
    return { 
      text: '', 
      error: `Failed to generate content after ${MAX_RETRIES} attempts. ${finalErrorMessage || 'Please try again later.'}` 
    };
  }
};
