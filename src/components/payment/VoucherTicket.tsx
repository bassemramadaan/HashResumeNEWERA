...
export default function VoucherTicket({
  ...
  return (
    ...
    >
      ...
      <div 
        className="relative flex items-stretch rounded-2xl shadow-xs border border-slate-200/60 text-slate-700 bg-white font-sans min-h-[190px]"
      >
        {/* Subtle decorative mesh overlay for refined textured design */}
        <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: "radial-gradient(#000 0.5px, transparent 0.5px)", backgroundSize: "12px 12px" }} />
        
        {/* SIDE A: Stub Part (Left for LTR, Right for RTL) */}
        <div className={`w-[30%] flex flex-col justify-between items-center p-3 sm:p-4 border-dashed border-slate-200 relative z-10 ${isAr ? "order-2 border-r" : "order-1 border-l-0"}`}>
          
          {/* Perforation Cutouts - exactly on the border edge */}
          <div className={`absolute -top-3 w-6 h-6 rounded-full bg-slate-50 z-20 shadow-inner border border-slate-200/45 ${isAr ? "-left-3" : "-right-3"}`} />
          <div className={`absolute -bottom-3 w-6 h-6 rounded-full bg-slate-50 z-20 shadow-inner border border-slate-200/45 ${isAr ? "-left-3" : "-right-3"}`} />

          {/* Vertical perforated line simulation */}
          <div className={`absolute top-0 bottom-0 ${isAr ? "left-0" : "right-0"} w-0 border-r border-dashed border-slate-300/40 z-10`} />

          {/* Ticket header tag */}
          <div className="text-center w-full mt-1">
            <span className="text-[8px] font-medium tracking-widest text-slate-400 uppercase block">
              {isAr ? "نوع الباقة" : "PLAN TYPE"}
            </span>
            <span className="text-[11px] font-semibold tracking-tight text-slate-600 mt-0.5 block">
              {type === "bundle" ? (isAr ? "المجموعة ٣" : "3-CODES") : (isAr ? "الفردية كود" : "SINGLE")}
            </span>
          </div>

          {/* Stylized realistic barcode but quieter */}
          <div className="flex flex-col items-center justify-center space-y-1 w-full my-2">
            <div className="flex items-end justify-center gap-[1.2px] py-1 px-1.5 bg-slate-50/50 rounded-md border border-slate-100">
              {[1, 2, 4, 1, 2, 3, 1, 1, 4, 2, 1, 3, 2, 1, 3, 1, 2, 1].map((n, idx) => (
                <div key={idx} className="bg-slate-400 rounded-full h-7" style={{ width: `${n * 0.6}px` }} />
              ))}
            </div>
            <span className="font-mono text-[8px] text-slate-400 select-none uppercase tracking-widest">
              {pendingRef ? `REF-${pendingRef}` : "SECURE"}
            </span>
          </div>
