...
const ResumePreview = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  return (
    <div 
      ref={ref} 
      className="w-full h-full bg-white text-slate-900 font-sans p-8 md:p-12 box-border"
    ...
    >
          ...
          <div className="space-y-3">
            {certifications.map((cert) => (
              <div key={cert.id} className="flex justify-between items-baseline">
                <div>
                  <h4 className="font-bold text-slate-800 inline">{cert.name}</h4>
                  <span className="text-sm font-medium text-slate-600 ml-2">— {cert.issuer}</span>
                </div>
                <span className="text-sm font-medium text-slate-500">{cert.date}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* Watermark */}
      <div className="mt-12 text-center text-xs text-slate-300 font-medium opacity-50">
