    ...
    return (
        <div className="flex flex-col items-center p-8 gap-8">
            <div className="flex gap-4 p-2 bg-neutral-100 rounded-lg">
                <button onClick={() => setTemplate("harvard")} className={`px-4 py-2 ${template === 'harvard' ? 'bg-white shadow' : ''}`}>Harvard</button>
                <button onClick={() => setTemplate("swiss")} className={`px-4 py-2 ${template === 'swiss' ? 'bg-white shadow' : ''}`}>Swiss</button>
                <button onClick={() => setTemplate("nordic")} className={`px-4 py-2 ${template === 'nordic' ? 'bg-white shadow' : ''}`}>Nordic</button>
            </div>
