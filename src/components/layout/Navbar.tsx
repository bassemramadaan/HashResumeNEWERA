...
export function Navbar({ onStartClick }: NavbarProps = {}) {
  ...
  const navItems = [
    {
      label: lang === 'ar' ? 'قوالب السيرة الذاتية' : lang === 'fr' ? 'Modèles' : 'Resume Templates',
      href: '/templates',
      icon: Layers,
      isInternal: true,
    ...
    {
      label: lang === 'ar' ? 'كيف يعمل؟' : lang === 'fr' ? 'Comment ça marche' : 'How it works',
      href: '/how-ats-works',
      icon: HelpCircle,
      isInternal: true
    },
    {
      label: lang === 'ar' ? 'الأسعار' : lang === 'fr' ? 'Tarifs' : 'Pricing',
      href: '/pricing',
      icon: CreditCard,
      isInternal: true
    }
  ...
  return (
    <nav className={cn(
      ...
      scrolled 
        ? "bg-white/80 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.03)] border-slate-200/80 py-1" 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between relative h-16">
          ...
          <div className="hidden lg:flex items-center justify-center gap-1.5 flex-1 px-4">
                ...
                {toolsOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute start-0 mt-2 bg-white border border-slate-200/80 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] overflow-hidden w-72 z-40 p-2.5 text-start"
                    >
                      {toolsMenu.items.map((item) => {
                        const isItemInternal = item.href.startsWith('/') && !item.href.includes('#');
                        const IconComponent = item.icon;
                        const linkClass = "flex items-start gap-3 p-2.5 rounded-xl hover:bg-slate-50/80 transition-all group";
                        ...
                        const content = (
                          <>
                            ...
                            <div className="flex-1 space-y-0.5 min-w-0">
                              ...
                              <p className="text-[10px] text-slate-400 font-medium leading-normal truncate">{item.desc}</p>
                            </div>
                          </>
                        );
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <IconComponent className="w-4 h-4 text-slate-400 group-hover:text-[#FF4D2D] transition-colors" />
                  <span>{item.label}</span>
                  
                  {item.badge && (
                    <span className={cn(
                      "text-[8px] font-black px-1.5 py-0.5 rounded-full leading-none transform scale-90",
        ...
        {mobileOpen && (
          ...
          >
            <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1.5">
              ...
              {/* Main Nav Items for Mobile with Icons */}
              {navItems.map((item) => {
                const IconComponent = item.icon;
                return (
                  <Link
                    ...
                    className="px-3.5 py-3 text-sm font-semibold text-slate-600 hover:text-slate-900 rounded-xl hover:bg-slate-50 flex items-center justify-between gap-2.5"
                  >
                    ...
                    {item.badge && (
                      <span className={cn(
                        "text-[8px] font-black px-2 py-0.5 rounded-full leading-none",
                        item.badgeColor === 'orange' ? "bg-[#FF4D2D] text-white" : "bg-emerald-50 text-emerald-600 border border-emerald-100"
                      )}>
                        {item.badge}
                      </span>
                    )}
              ...
              <div className="mt-3 pt-3 border-t border-slate-100">
                <div className="px-3 text-[10px] font-black text-slate-400 tracking-wider uppercase mb-2">
                  {toolsMenu.label}
                </div>
                {toolsMenu.items.map((item) => {
                  const isItemInternal = item.href.startsWith('/') && !item.href.includes('#');
                  const IconComponent = item.icon;
                  const linkClass = "px-4 py-3 text-sm font-semibold text-slate-500 hover:text-slate-900 rounded-xl hover:bg-slate-50/70 flex items-center justify-between gap-2.5";
