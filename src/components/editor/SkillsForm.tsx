    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100">
        <form onSubmit={handleAdd} className="mb-6">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 sm:text-sm transition-colors"
              placeholder="e.g. React, Project Management..."
            />
            <button
              type="submit"
              disabled={!inputValue.trim()}
              className="bg-green-500 hover:bg-green-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-4 py-2.5 rounded-xl font-medium transition-colors flex items-center gap-2"
            >
              <Plus size={18} />
        ...
        {unaddedSuggestions.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-slate-500 mb-3">Suggested Skills</h3>
            <div className="flex flex-wrap gap-2">
              {unaddedSuggestions.map((skill) => (
                <button
                  key={skill}
                  onClick={() => addSkill(skill)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-green-50 hover:text-green-700 hover:border-green-200 text-sm font-medium transition-colors"
                >
                  <Plus size={14} />
                  {skill}
                </button>
