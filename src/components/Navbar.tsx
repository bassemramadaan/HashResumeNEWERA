...
import HashHuntModal from "./HashHuntModal";
...
export default function Navbar() {
  ...
  const [isHashHuntModalOpen, setIsHashHuntModalOpen] = useState(false);
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div
            className={cn(
              ...
              isScrolled
                ? "bg-white/90 backdrop-blur-xl shadow-lg shadow-slate-200/20 border border-slate-200/20"
                : "bg-transparent",
            )}
          >
            {/* Left Navigation (Desktop) */}
            <div className="flex items-center">
              <nav className="hidden md:flex items-center gap-2 bg-slate-100/50 p-1.5 rounded-full border border-slate-200/50 w-fit">
                {navLinks.map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                  ...
                  <AnimatePresence>
                    {isMoreOpen && (
                      <motion.div
                        ...
                        className="absolute top-full start-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden py-2 z-50 text-start"
                      >
                        {moreLinks.map((link) => (
                          <Link
                            key={link.path}
                            to={link.path}
                            onClick={link.name === (t.landing.hashHuntJobs || "Hash Hunt") ? (e) => {
                              if(location.pathname === "/") {
                                e.preventDefault();
                                setIsHashHuntModalOpen(true);
                                setIsMoreOpen(false);
                            ...
                            className={cn(
                              "px-4 py-3 text-sm font-bold transition-all flex items-center justify-between hover:bg-slate-50",
            ...
            <div className="flex justify-center">
              <Link to="/" className="flex items-center gap-1 flex-shrink-0 group">
                <Logo width="auto" height={160} variant="gradient" className="transition-all duration-300 hover:scale-105 h-16 sm:h-24 md:h-28 lg:h-32" />
              </Link>
            </div>

            {/* Actions (Right) */}
            <div className="flex items-center justify-end gap-2 sm:gap-4">
              <div className="hidden sm:flex items-center gap-2 bg-slate-100/50 p-1 rounded-full border border-slate-200/50 ">
                <LanguageSwitcher />
              </div>

              <Link
                to="/editor"
                className="flex items-center gap-2 bg-gradient-to-b from-[#ff4d2d] to-orange-600 shadow-[0_8px_16px_-6px_rgba(255,77,45,0.5),inset_0_2px_0_rgba(255,255,255,0.2)] text-white font-bold py-2.5 px-4 sm:px-6 rounded-full hover:scale-105 active:scale-95 transition-all group animate-[pulse_3s_infinite] min-h-[44px]"
              >
                <Sparkles
                  size={16}
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="md:hidden absolute top-full start-4 end-4 mt-2 bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden z-50 max-h-[80vh] overflow-y-auto"
              >
                <div className="p-6 flex flex-col gap-4">
                  ...
                  {[...navLinks, ...moreLinks].map((link, i) => (
                    ...
                    >
                      <Link
                        to={link.path}
                        onClick={(e) => {
                          if (link.name === (t.landing.hashHuntJobs || "Hash Hunt") && location.pathname === "/") {
                            e.preventDefault();
                            setIsHashHuntModalOpen(true);
                          }
                  ...
                  <Link
                    to="/editor"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center justify-center gap-3 bg-brand-600 text-white font-bold py-4 px-6 rounded-2xl text-center active:scale-95 transition-all shadow-lg shadow-brand-100"
                  >
      ...
      <HashHuntModal 
        isOpen={isHashHuntModalOpen} 
        onClose={() => setIsHashHuntModalOpen(false)} 
      />
    </>
  );
}
