...
@theme {
  ...
  --font-display: 'Inter', 'IBM Plex Sans Arabic', sans-serif;
`;

const rootStart = indexCss.indexOf('/* ============================================================');
const shadowsStart = indexCss.indexOf('/* Shadows */');

const firstPart = indexCss.slice(0, rootStart);
const lastPart = indexCss.slice(shadowsStart);

fs.writeFileSync('src/index.css', firstPart + newTokens + '\n  ' + lastPart, 'utf8');
console.log('done');
